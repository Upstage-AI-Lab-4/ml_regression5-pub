...
- 제공된 data와 baseline_code를 바탕으로 서울의 아파트의 해당 시점의 매매 실거래가를 예측하는 Regression 대회로 평가지표는 RMSE(Root Mean Squared Error)를 사용
![RMSE](./img/rmse.png)
...
1-1. Database 에 train.csv를 import
...
1-4. 팀 공유
    1) git에 올릴 경우 데이터 용량 이슈로 인해 csv 파일 형태가 아니라 압축하여 올려야 함
    2) 공유된 파일 
...
![재건축연한 조건](./img/rebuild_condition.png) ![재건축연한여부 파생변수 추가](./img/rebuild.png)
